    ...
    <AppShell title="Faturas">
      ...
      {isLoading ? (
        ...
        <div className="text-center py-12 text-muted-foreground text-sm flex flex-col items-center gap-2">
          ...
          {faturas.length === 0
            ? "Nenhuma fatura ainda. Gere uma a partir de uma OS."
            : "Nenhum resultado."}
        </div>
      ) : (
        <div className="grid gap-2">
          {filtered.map((f) => (
            ...
            >
              ...
              >
                <div className="font-mono text-sm font-semibold w-20 shrink-0">
                  {f.numero}
                </div>
      ...
      <NovaFaturaDialog
        ...
        onCreated={() => qc.invalidateQueries({ queryKey: ["faturas"] })}
      />
  ...
  const { data: ordens = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from("ordens_servico")
        .select("id, numero, total, cliente_id, cliente:clientes(nome)")
        .order("created_at", { ascending: false })
        .limit(100);
      ...
      <DialogContent>
        ...
        <div className="space-y-3">
          <div className="space-y-1.5">
            <Label>A partir de OS (opcional)</Label>
            <Select value={osId} onValueChange={setOsId}>
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="_none">— Fatura em branco —</SelectItem>
                {ordens.map((o) => (
                  <SelectItem key={o.id} value={o.id}>
                    #{o.numero} — {o.cliente?.nome ?? "sem cliente"} ·{" "}
                    {brl(Number(o.total))}
                  </SelectItem>
                ))}
              </SelectContent>
